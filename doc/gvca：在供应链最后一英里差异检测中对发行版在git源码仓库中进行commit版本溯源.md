...
在开源软件供应链的交付阶段，开源软件维护者将source code repository打包为source distribution并构建为built distribution，交付到开源软件软件包分发平台。Although developers overwhelmingly obtain dependencies from these distribution platforms, the released distributions receive disproportionately lower scrutiny than their upstream version control repositories. This disparity arises from the widespread yet flawed assumption that source distributions faithfully mirror the corresponding upstream commit—a transitive trust that has been repeatedly exploited in real-world supply chain attacks (e.g., XZ Utils backdoor in 2024).

While reproducible builds, which establish provenance from source distributions to built distributions, have been extensively studied and are becoming industry practice, establishing cryptographic provenance from an upstream VCS commit to its published source distribution remains a largely unsolved challenge. A fundamental difficulty lies in version alignment: given a source distribution with a specific version number, reliably identifying the exact upstream commit(s) from which it was packaged.

We present GVCA(Git Version-to-Commit Aligner), a novel version alignment methodology. GVCA constructs reverse indexes over VCS histories to map source distributions back to their originating commits. We apply GVCA to 3,920 of the most widely used PyPI packages and their declared upstream repositories. Whereas conventional tag-based heuristics fail on 1,042 packages (26.6%),我们的方法能够将不可识别数量降低至xxx，使得识别率最终提升至xxx。Our results and artifacts are publicly available.

## Introduction

The auditability and transparency of Free and Open Source Software (FOSS) are foundational to its security model. Security assessment of FOSS has thus primarily focused on human review and automated scanning of upstream version control system (VCS) repositories, establishing commit history and pull-request workflows as the predominant, high-effort checkpoints in the supply chain. In practice, however, the artifacts actually consumed by downstream users—the released source and built distributions—receive dramatically less scrutiny.

In the delivery phase of open-source supply chains, maintainers package code from upstream VCS repositories into source distributions, which are uploaded to public distribution platforms (e.g., PyPI, npm, Debian repositories) and subsequently used to generate built distributions (wheels, debs, rpms, etc.). The absence of verifiable provenance linking reviewed upstream commits to released distributions turns this phase into an attractive attack surface. The 2024 XZ Utils backdoor exemplified this risk: a malicious maintainer injected malicious artifacts exclusively into the released source distributions, bypassing all VCS-level review and triggering remote code execution during the build process of downstream consumers.
...
通过对PyPI最受欢迎的可获取其原始仓库的前4000个包进行了启发式的commit来源分析，有26.6%的交付源码包无法通过启发式方法获得其在源码仓库中的commit来源：
...
\begin{algorithmic}[1] % 启用行号
\Require Inverse relation \(\mathcal{F}^{-1} \subseteq \mathcal{B} \times \mathcal{P} \times \mathcal{C}\)
...
flowchart TD
    Start(["Start"])
    Input[/"Input inverse relation <br>𝓕⁻¹ ⊆ 𝓑×𝓟×𝓒"/]
...
- 将得到的匹配对 $\{(b_i, p_i)\}$ 按路径排名排列（依据路径对应的不同blob版本数量降序排名，优先选择高变异性路径以加速收敛）。

这将生成一个有序的贡献者 $(b, p)$ 对列表，随后通过反向关系索引 $\mathcal{F}^{-1}$ 查询对应的提交集合。

#### 核心过滤算法

筛选过程从包含所有提交的初始候选集 $  \mathcal{C}  $ 开始，依次对每个 $  (b_i, p_i) \in R  $ 与 $  \mathcal{F}^{-1}(b_i, p_i)  $ 取交集。为处理文件来源不一致（如来自不相交commit），gvca维护了一个候选集列表 $  \mathcal{S}  $。对于每个 $  (b_i, p_i)  $，gvca选择性地精炼候选集：若某候选集 $ C $ 的交集 $ C' = C \cap \mathcal{F}^{-1}(b_i, p_i) $ 非空，则更新该候选集为 $   C'   $；若交集为空，则保留原候选集不变。若所有候选集的交集均为空，则从 $  \mathcal{F}^{-1}(b_i, p_i)  $ 创建新候选集，并从 $ R $ 开头到当前位置重新筛选以确保一致性。

\begin{algorithm}
\caption{DynamicCommitFiltering}
\begin{algorithmic}[1] % 启用行号
\Require Preprocessed release package \Statex \( R = \{(b_i, p_i), \dots, (b_n, p_n)\} \), sorted by path ranking in descending order of blob version count
\Require Inverse relation index \( \mathcal{F}^{-1} \subseteq \mathcal{B} \times \mathcal{P} \times \mathcal{C} \)
\Ensure List of candidate commit sets \(\mathcal{S}\)
\State \(\mathcal{S} \gets \emptyset\) \Comment{Init empty candidate set list}
\For{each \((b_i, p_i) \in R\)} \Comment{Iterate through pairs in ranking order}
    ...
    \If{\(\mathcal{S} = \emptyset\)} \Comment{Handle first or empty set}
        \State \(NewC \gets \mathcal{F}^{-1}(b_i, p_i)\)
        \If{\(NewC \neq \emptyset\)}
    ...
    \Else
        \For{each \(C_k \in \mathcal{S}\)} \Comment{Process each candidate set}
            \State \(C_k' \gets C_k \cap \mathcal{F}^{-1}(b_i, p_i)\) \Comment{Compute intersection}
            \If{\(C_k' \neq \emptyset\)}
    ...
    \If{\(\neg hasNonEmptyIntersection\)} \Comment{Create new set if all intersections empty}
        \State \(NewC \gets \mathcal{F}^{-1}(b_i, p_i)\)
        \If{\(NewC \neq \emptyset\)}
            \For{each \((b_j, p_j) \in R\) from index 1 to current index - 1} \Comment{Rescreen prior pairs}
                \State \(NewC' \gets NewC \cap \mathcal{F}^{-1}(b_j, p_j)\)
                \If{\(NewC' \neq \emptyset\)}
...
flowchart TD
    Start([Start])
    InputR[/"Input preprocessed release package <br>R = {(b₁,p₁),…,(bₙ,pₙ)}"/]
    InputInverse[/"Input inverse relation index 𝓕⁻¹ ⊆ 𝓑×𝓟×𝓒"/]
    InitS["Initialize 𝓢 ← ∅"]
    LoopR{{"For each (bᵢ,pᵢ) ∈ R <br>(by ranking order)"}}
    InitLoopR["𝓢′ ← ∅, <br>hasNonEmpty ← False"]
    CheckS{"If 𝓢 = ∅"}
    FirstSBranch["NewC ← 𝓕⁻¹(bᵢ,pᵢ)"]
    FirstSCAppend["If NewC ≠ ∅ then <br>append to 𝓢′, <br>set hasNonEmpty ← True"]
    LoopSBranch{{"For each Cₖ ∈ 𝓢"}}
    Intersection["C′ ← Cₖ ∩ 𝓕⁻¹(bᵢ,pᵢ)"]
    ...
    AllEmpty["NewC ← 𝓕⁻¹(bᵢ,pᵢ)"]
    Rescreen["If NewC ≠ ∅ then for j < i: NewC ← NewC ∩ 𝓕⁻¹(bⱼ,pⱼ)"]
...
`cucumber-expressions`包、`delta-sharing`、`dify-plugin`、`fastdiff`、`gherkin-official`、`graphframes`、`mujoco`、`phonenumbers`、`phonenumberslite`、`py-vapid`包：release包实际对应于repo的子目录`python`，但由于元数据未标注这一点，导致对齐错误。
...
`kestra-io`包实际对应于repo`kestra-io/libs`的子目录`python`，元数据未标注。
`kfp-server-api`最新版本的包实际对应于repo`kubeflow/pipelines`的子目录`backend/api/v2beta1/python_http_client`，元数据未标注。
`langchain-cohere`包实际对应于repo`langchain-ai/langchain-cohere`的子目录`libs/cohere`，元数据未标注。
`langchain-experimental`包实际对应于repo`langchain-ai/langchain-experimental`的子目录`libs/experimental`，元数据未标注。
`langchain-nvidia-ai-endpoints`包实际对应于repo`langchain-ai/langchain-nvidia`的子目录`/libs/ai-endpoints/`，元数据未标注。
`livekit`、`livekit-agent`、`livekit-api`、`livekit-plugins-deepgram`、`livekit-plugins-openai`、`livekit-plugins-silero`、`livekit-plugins-turn-detector`、`livekit-protocol`包实际分别对应于repo`livekit/python-sdks`的子目录`livekit-rtc`、`livekit-agents`、`livekit-api`、`livekit-plugins/livekit-plugins-deepgram`、`livekit-plugins/livekit-plugins-openai`、`livekit-plugins/livekit-plugins-silero`、`livekit-plugins/livekit-plugins-turn-detector`、`livekit-protocol`，元数据未标注。
`magika`包实际对应repo`google/magika`的子目录`python`。
`markitdown`包实际对应于repo`microsoft/markitdown`的子目录`packages/markitdown`。
`mcap`包实际对应repo`foxglove/mcap`的子目录`python/mcap`。
`opentelemetry-instrumentation-click`、`opentelemetry-instrumentation-threading`、`opentelemetry-test-utils`包分别实际对应repo`open-telemetry/opentelemetry-python-contrib`的子目录`instrumentation/opentelemetry-instrumentation-click`、`instrumentation/opentelemetry-instrumentation-threading`、`tests/opentelemetry-test-utils`，元数据标注为`https://github.com/open-telemetry/opentelemetry-python-contrib/instrumentation/opentelemetry-instrumentation-click`、`https://github.com/open-telemetry/opentelemetry-python-contrib/instrumentation/opentelemetry-instrumentation-threading`、`https://github.com/open-telemetry/opentelemetry-python/tests/opentelemetry-test-utils`，这些url已经无法访问，因此虽然元数据表达了子目录信息但未能成功解析。
`pycocotools`包对应repo`ppwwyyxx/cocoapi`的子目录`PythonAPI`
`pydantic-evals`、`pydantic-graph`包分别对应repo`pydantic/pydantic-ai`的子目录`pydantic-evals`、`pydantic_graph`
`pyhanko`包对应repo`MatthiasValvekens/pyHanko`的子目录`pkgs/pyhanko`
`pyobjc`、`pyobjc-core`...包分别对应repo`ronaldoussoren/pyobjc`的子目录`pyobjc`、`pyobjc-core`...
...
`kpf`包：release实际对应于repo`kubeflow/pipelines`的子目录`sdk/python`，元数据错误地标注子目录为`sdk`导致无法匹配。

#### 因目录组织重构引发的不良或半不良报告

一些repo的目录组织在实际release的组织中发生了重构。尽管仍有包括`README.md`、`pyproject.toml`、`docs\*`等部分可供进行校验的目录重合工件，但是多数工件，尤其是`.py`工件的所属目录在repo打包为release时进行了迁移。
...
`lightgbm`包的`src`目录可以与repo`microsoft/LightGBM`的对应目录正确映射，但是其中不包含关键的`.py`工件。release包下的`.py`工件实际对应原始仓库的`python-package`目录。此外release包下的`external_libs/*`工件同样对应原始仓库的其他目录。
`mmcif-pdbx`包的`pdbx`目录下的文件实际上来自于`Electrostatics/mmcif_pdbx`仓库下多个不同目录。
`patchelf`包的repo`NixOS/patchelf`中的主目录内容在分发时被重构到了分发包的`src/patchelf-upstream`目录下。
`prettierfier`包的repo中的主目录内容在分发时被重构到了分发包的`prettierfier`目录下。
`primepy`包的repo中的主目录内容在分发时被重构到了分发包的`primePy`目录下。
`pyprmor`、`pyarmor-cli-core`包的子目录`pyarmor`映射自主目录的`src`目录。
`pymupdf4llm`包的repo主目录内容分发市重构到`pymupdf4llm`目录下。
`qprompt`包的repo主目录内容分发市重构到`lib`目录下。

#### CRLF问题

`debugpy`、`diff-parser`、`django-json-widget`、`docx2pdf`、`easygui`、`editables`、`filigran-sseclient`、`flask-oauthlib`、`formic2`、`gitdb2`、`gputil`、`graphemeu`、`graphviz`、`imap-tools`、`interegular`、`jschema-to-python`、`json-logging`、`json-formatter`、`jsons`、`keybert`、`kivy`、`korean-lunar-calendar`、`markdown-pdf`、`mistleoe`、`mouseinfo`、`opencensus`、`opencensus-context`、`opencensus-ext-logging`、`opentelemetry-propagator-aws-xray`、`optimizely-sdk`的release包的绝大多数文本文件以CRLF结尾，而repo的所有文本文件以LF结尾，导致无法匹配。
`numdifftools`、`pandas-market-calendars`、`pathlib2`、`piexif`、`ping3`、`pkce`、`pretty-html-table`、`pyautogui`、`pybtex-docutils`、`pybytebuffer`、`pydevd`、`pygetwindow`、`pyiso8583`、`pymarkdownlnt`、`pyrect`、`pyscreeze`、`pysfeel`、`pytest-watch`、`python-tds`、`python-xlib`、`pytweening`、`pywin32-ctypes`的release包的绝大多数文本工件以CRLF结尾，repo的绝大多数文本工件以LF结尾。但是，意外地，repo中的部分文本文件能够匹配release包的以CRLF结尾的工件，导致能够部分匹配得到相对良性的报告。

`pymannkendall`多数文本工件以CRLF结尾，部分以LF结尾，以CRLF结尾的工件成为无法匹配的幽灵文件。
...
`kfp-pipeline-spec`包整个都是自动生成的，原始仓库中已经找不到其核心内容原本对应的文件了。
...
`localstack`包仅仅为了引入`localstack-core`和`localstack-ext`的依赖，包含少量文件也经历了目录名重构。
`mypy-boto3*`系列400多个包均为原始仓库代码生成的，原始仓库几乎没有出现分发包内容文件。
`protoc-gen-openapiv2`整个分发包的代码几乎均出自代码生成。
`ppymorphy3-dicts-ru`的分发包内容可能来自自动生成。
`pyqt5-sip`、`pyqt6-sip`的分发包内容可能来自代码生成。

#### 不正确的仓库元数据

`microsoft-kiota-*`系列包源码已经迁移到`microsoft/kiota-python`仓库，但是元数据仍然标注为`microsoft/kiota`仓库。

### 不良报告的自动化判定

#### 易被错误分类的不良报告

经过对报告的手工分析，存在少量不良报告因最低候选集未超过阈值而被错误地判定为良好报告的情形。

`phonenumberslite`是一个典型案例：根据启发式分析方法知，它的实际commit版本为`e0f120940ef95ae8cf3fafc2216de50606ebdeed`，但是该commit位于包含112个候选集0中。而候选集1仅有4个commit，极易被认为是结果数量未超出阈值的良好报告。实际上，导致这种情况的真实原因是：早期仓库的目录与分发包目录组织相同，后来，分发包组织目录的对应内容被转移到仓库的`python`子目录下。因此，这本该导致出现分发包与对应仓库子目录不匹配的问题，却因为`phonenumbers/__init__.pyi`、`phonenumbers/shortdata/__init__.pyi`、`phonenumbers/carrierdata/__init__.pyi`、`phonenumbers/geodata/__init__.pyi`、`phonenumbers/tzdata/__init__.pyi`这几个文件可以与仓库早期版本的路径与内容错误地完全匹配，得到了看似合理实则错误的commit候选集。这警示对于多候选集的报告结果需要更慎重的审查。

### 预处理仓库数据统计 commit数量表格 (p,b)对数量表格。

### 性能

实验部署的服务器配置。采用参数：核心/rocksdb的线程数。

#### 性能图1：横坐标：预处理时间段 纵坐标： 包数量

解释：整体4000个包的工作的包花费的时间的分布。大约xx%的包耗时在1秒以内，yy%的包耗时在1分钟以内，zz%的包耗时在1小时以内。

#### 性能图2： 横坐标：commit数量 纵坐标： 预处理时间

解释：预处理时间与vcs仓库commit数量的关系。[对于commit数量在xx~yy范围内，预处理时间平均为zz，最大不超过tt。]对于包集中commit最多的包的预处理时间在。

#### 性能图3： 横坐标：(p,b)对数量 纵坐标： 预处理时间

解释：预处理时间与(p,b)对数量的关系。

#### 和lastPymile与git log的特定包性能对比表格

性能占优势

描述git log方案下对于几百量级的commit的差异分析处理时间，比较本方案在monorepo极大量级情况下的处理时间。表格基础上加一列量级（commit数量）
...
所有包的属性统计。commit数量分布/(path, blob)对数量分布，排除（xx、xx原因导致异常）的成功包的数量。

包的文件路径后缀分析：.py、.rs、.h、.c数量。

异常原因具体内容见后章节

#### 成功包的commit筛选数量表格 是commit数量1/2/3/4/5/6~10/11~20/20~50/50~100/100，纵坐标是包个数

##### 导致commit数量多的path phatom文件。导致commit数量多的dismatch phatom文件的图

#### 成功包的候选集数量 体现了版本漂移

##### commit中多候选集中，仅在仓库更改而source distribution未更改的情形

##### 实际举例： 表格：工程名 版本漂移文件内容
